...
function AddSection() {
 ...
 const [sectionName, setSectionName] = useState("");
  ...
  const getSectionById = async () => {
    ...
    setTableData(data?.section_dates[0]?.section_details)
  };
  useEffect(() => {
    if (id) {
      console.log(id);
  ...
  const selectedDateObject = sectionDates?.find((item) => item.date === selectedDate);
  useEffect(() => {
  ...
  return (
    <div className="add-section-container">
      ...
      <div className="input-container">
        ...
        <button onClick={()=>{setShow(!show)}}>Add Row</button>
        <div>
            ...
            <tbody>
              {tableData?.map((row, index) => {
                return (
                  ...
                  <td>
                    {editableRow === index ? (
                      <input
                        type="date"
                        value={row.date}
                        onChange={(e) => handleInputChange(e.target.value, 'date', index)}
                      />
                    ) : (
                      selectedDate
                    )}
                  ...
                  <td>
                    {editableRow === index ? (
                      <input
                        type="text"
                        value={row.shift_id}
                        onChange={(e) => handleInputChange(e.target.value, 'shift_id', index)}
                      />
                    ) : (
                      row.shift_id
                    )}
                  ...
                  <td>
                    {editableRow === index ? (
                      <select
                        value={row.staff_ids}
                        onChange={(e) => handleInputChange(e.target.value, 'staff_ids', index)}
                      >
                        {users.map((user) => (
                ...
                row?.users?.map((staffId) => {
                        const staff = users.find((user) => user.id === staffId.id);
     ...
     {show  &&   <YourModal id={id} closeModal={()=>{setShow(false)}} update={()=>{getSectionById()}}/>}
    </div>
  );
}
